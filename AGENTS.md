...
Use the assigned aggregate compute budget (currently 16 cores) for parallelizable work. Backend analysis and publisher/overview share gaia-compute.slice (CPUQuota=1600%); do not independently exceed that allocation. Star fits run in parallel by catalogue chunk, preserve deterministic order, and are forbidden when solar elevation is above 0 degrees. Daylight acquisition/projection remain enabled. Ready cloud fields arrive atomically from the bounded background star pass; the realtime publisher consumes them without waiting.
