...
export default function Home() {
  ...
  async function setQuality(camera:Camera,quality_exponent:number){setCameras(rows=>rows.map(row=>row.id===camera.id?{...row,quality_exponent}:row));const response=await fetch(`/gaia/api/sources/${encodeURIComponent(camera.id)}/settings`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({quality_exponent})});if(!response.ok){setCameras(rows=>rows.map(row=>row.id===camera.id?{...row,quality_exponent:camera.quality_exponent}:row));alert('Could not save the quality weight.')}}
