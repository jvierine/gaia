...
import {ServiceDescription,AuthorAffiliations,ProviderContacts} from '../src/ProviderContacts';
...
type ViewName = 'globe' | 'cameras' | 'status' | 'calibrate' | 'events' | 'about';
  ...
  return <main className="app-shell">
    ...
    <section className="workspace viewer-only">
      <section className="viewer">
        ...
        {view === 'events' && <div className="route-panel narrow"><div className="route-heading"><CalendarDays/><div><span className="eyebrow">AURORAL EVENT STUDIES</span><h2>Events</h2></div></div><p>Explore time-located photographs and timelapses from major geomagnetic storms. Event studies are separate from GAIA’s realtime camera database and projection pipeline.</p><div className="event-catalogue"><article><div className="event-catalogue-date"><strong>11–12</strong><span>NOV 2025</span></div><div><span className="eyebrow">G4 GEOMAGNETIC STORM</span><h3>The 11–12 November 2025 auroral storm</h3><p>1,230 located observations, including 24 timelapses, arranged by exact UTC time and geographic position.</p><a href="/gaia/events/20251111/">Open event study <span aria-hidden="true">→</span></a></div></article></div></div>}
        {view === 'about' && <div className="route-panel narrow"><div className="route-heading"><CircleHelp/><div><span className="eyebrow">OPEN SCIENCE INFRASTRUCTURE</span><h2>About the data center &amp; credits</h2></div></div><ServiceDescription/><h3>Authors</h3><AuthorAffiliations/><p>GAIA is an independent image aggregation and geographic projection service.</p><p>Image copyrights remain with their producers. GAIA does not transfer ownership or replace the producer’s terms.</p><h3>Suggest an improvement</h3><p>Report a bad image, missing source, calibration issue or viewer idea.</p><button className="suggest-link" onClick={()=>{setSuggesting(true);setSent(false)}}><Lightbulb size={14}/> Suggest an improvement</button><ProviderContacts/><Credits/></div>}
