...
function MaskEditor({camera,onClose}:{camera:Camera,onClose:()=>void}){
  ...
  const[points,setPoints]=useState<[number,number][]>([]);const[crop,setCrop]=useState([0,0,100,100]);const[saving,setSaving]=useState(false);const imageRef=useRef<HTMLImageElement>(null);const drag=useRef<{kind:'node'|'crop';edge?:string;index?:number;x:number;y:number;points:[number,number][];crop:number[]}|null>(null);
  ...
  const pointerDown=(event:React.PointerEvent<HTMLDivElement>)=>{if(!imageRef.current||event.button!==0)return;event.preventDefault();const target=event.target as HTMLElement,kind=target.dataset.kind,[x,y]=normalized(event);event.currentTarget.setPointerCapture(event.pointerId);if(kind==='node'&&mode==='mask')drag.current={kind:'node',index:Number(target.dataset.index),x,y,points,crop};else if(kind==='crop'&&mode==='crop')drag.current={kind:'crop',edge:target.dataset.edge,x,y,points,crop};else if(mode==='mask')setPoints(p=>[...p,[Math.max(-.25,Math.min(1.25,x)),Math.max(-.25,Math.min(1.25,y))]])};
