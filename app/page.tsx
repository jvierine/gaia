...
function Globe({epochMillis,live,onCredits,onLoading,sunLock}:{epochMillis:number;live:boolean;onCredits:()=>void;onLoading:(loading:boolean)=>void;sunLock:boolean}) {
  return <GaiaGlobeView className="globe-stage" getEpochMillis={()=>live?Date.now():epochMillis} onLoading={onLoading} sunLock={sunLock}>
...
export default function Home() {
  ...
  useEffect(()=>{if(!playing||framesLoading||view!=='globe')return;const timer=window.setTimeout(()=>setTimeMinutes(v=>historyMinutes.find(m=>m>v)??historyMinutes[0]??1440),150);return()=>window.clearTimeout(timer)},[playing,framesLoading,timeMinutes,view,historyMinutes]);
  ...
  return <main className="app-shell">
    ...
    <section className="workspace viewer-only">
      <section className="viewer">
        ...
        <div className="source-strip"><div><Database size={15} /><span><strong>{playing?'Smoothed playback':timeMinutes===1440?'Live frames':'Archived frames'}</strong> {framesLoading?'Loading…':'Selected at or before timeline time; gaps over 10 min omitted'}</span></div><div className="legend"><span className="dot live" /> clear <span className="dot delay" /> delayed <span className="dot idle" /> unavailable</div></div>
      </section>
    </section>
    <footer><span>GAIA Data Center</span></footer>
