...
function Globe({epochMillis,live,onCredits,onLoading,sunLock,zoomRef}:{epochMillis:number;live:boolean;onCredits:()=>void;onLoading:(loading:boolean)=>void;sunLock:boolean;zoomRef:{current:((action:'in'|'out'|'reset')=>void)|null}}) {
  return <GaiaGlobeView className="globe-stage" getEpochMillis={()=>live?Date.now():epochMillis} onLoading={onLoading} sunLock={sunLock} showTools={false} zoomRef={zoomRef}>
  ...
  return <main className="app-shell">
    ...
    <section className="workspace viewer-only">
      <section className="viewer">
        ...
        <div className="source-strip">{view==='globe'&&<div className="strip-zoom" role="group" aria-label="Map controls"><button type="button" aria-label="Zoom in" title="Zoom in" onClick={()=>zoomRef.current?.('in')}>+</button><button type="button" aria-label="Zoom out" title="Zoom out" onClick={()=>zoomRef.current?.('out')}>&minus;</button><button type="button" aria-label="Reset globe" title="Reset view" onClick={()=>zoomRef.current?.('reset')}>◎</button></div>}<div><Database size={15} /><span><strong>{playing?'Smoothed playback':timeMinutes===1440?'Live frames':'Archived frames'}</strong> {framesLoading?'Loading…':'Selected at or before timeline time; gaps over 10 min omitted'}</span></div><div className="legend"><span><span className="dot live" /> clear</span><span><span className="dot delay" /> delayed</span><span><span className="dot idle" /> unavailable</span></div></div>
      </section>
    </section>
