...
export default function Home() {
  ...
  useEffect(()=>{const update=(event:Event)=>{const {id,enabled}=(event as CustomEvent<{id:string;enabled:boolean}>).detail;setCameras(rows=>rows.map(row=>row.id===id?{...row,enabled}:row));};window.addEventListener('gaia-camera-state',update);return()=>window.removeEventListener('gaia-camera-state',update)},[]);
