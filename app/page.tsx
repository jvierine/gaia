...
function MaskEditor({camera,onClose}:{camera:Camera,onClose:()=>void}){
  const[points,setPoints]=useState<[number,number][]>([]);const[crop,setCrop]=useState([0,0,100,100]);const[saving,setSaving]=useState(false);
  const addPoint=(event:React.PointerEvent<HTMLDivElement>)=>{const r=event.currentTarget.getBoundingClientRect();setPoints(p=>[...p,[(event.clientX-r.left)/r.width,(event.clientY-r.top)/r.height]])};
  ...
  return <div className="modal-backdrop"><div className="mask-editor"><button className="close" onClick={onClose} aria-label="Close"><X/></button><span className="eyebrow">CROP &amp; OBSTRUCTION MASK</span><h2>{camera.name}</h2><p>Click around trees, buildings or other blocked parts of the image to define a polygon. Crop values are percentages of image width and height.</p><div className="mask-image" onPointerDown={addPoint}><img src={`/gaia/api/sources/${camera.id}/latest`} alt={`Latest frame from ${camera.name}`}/><svg viewBox="0 0 1 1" preserveAspectRatio="none"><polygon points={points.map(p=>p.join(',')).join(' ')}/>{points.map((p,i)=><circle key={i} cx={p[0]} cy={p[1]} r=".008"/>)}</svg></div><div className="crop-grid">{['Left','Top','Right','Bottom'].map((label,i)=><label key={label}>{label}<input type="number" min="0" max="100" step="0.1" value={crop[i]} onChange={e=>setCrop(v=>v.map((x,j)=>j===i?Number(e.target.value):x))}/></label>)}</div><div className="mask-actions"><button onClick={()=>setPoints(p=>p.slice(0,-1))}>Undo point</button><button onClick={()=>setPoints([])}>Clear mask</button><button className="send-button" onClick={save} disabled={saving}>{saving?'Saving…':'Save crop and mask'}</button></div></div></div>
}
