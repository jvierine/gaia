...
function MaskEditor({camera,onClose}:{camera:Camera,onClose:()=>void}){
  ...
  useEffect(()=>{
    ...
    void fetch(`/gaia/api/sources/${encodeURIComponent(camera.id)}/settings`,{cache:'no-store',signal:controller.signal}).then(async response=>{
      ...
      setPoints(settings.mask?.polygons?.[0]??[]);
      setLoading(false);
    }).catch(error=>{if(!controller.signal.aborted){setLoadError(String(error.message));setLoading(false)}});
    return()=>controller.abort();
  },[camera.id]);
  const[points,setPoints]=useState<[number,number][]>([]);const[crop,setCrop]=useState([0,0,100,100]);const[saving,setSaving]=useState(false);const imageRef=useRef<HTMLImageElement>(null);const drag=useRef<{kind:'node'|'crop';index?:number;x:number;y:number;points:[number,number][];crop:number[]}|null>(null);
  ...
  return <div className="modal-backdrop"><div className="mask-editor"><button className="close" onClick={onClose} aria-label="Close"><X/></button><span className="eyebrow">CROP &amp; OBSTRUCTION MASK</span><h2>{camera.name}</h2><p>Click to add mask nodes; drag any node to refine it. The editing margin accepts nodes outside the image so edge masks are easy to close. Drag the cyan crop box by its solid top handle or edit its exact percentages.</p><div className="mask-image" onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={()=>drag.current=null} onPointerCancel={()=>drag.current=null}><div className="mask-frame"><img ref={imageRef} src={`/gaia/api/sources/${camera.id}/latest`} alt={`Latest frame from ${camera.name}`}/><svg viewBox="0 0 1 1" preserveAspectRatio="none"><polygon points={points.map(p=>p.join(',')).join(' ')}/><rect className="crop-box" data-kind="crop" x={crop[0]/100} y={crop[1]/100} width={(crop[2]-crop[0])/100} height={(crop[3]-crop[1])/100}/><rect className="crop-grab" data-kind="crop" x={crop[0]/100} y={crop[1]/100} width={(crop[2]-crop[0])/100} height=".035"/>{points.map((p,i)=><circle className="mask-node" data-kind="node" data-index={i} key={i} cx={p[0]} cy={p[1]} r=".012"/>)}</svg></div></div><div className="crop-grid">{['Left','Top','Right','Bottom'].map((label,i)=><label key={label}>{label}<input type="number" min="0" max="100" step="0.1" value={Number(crop[i].toFixed(1))} onChange={e=>setCrop(v=>v.map((x,j)=>j===i?Number(e.target.value):x))}/></label>)}</div><div className="mask-actions"><button onClick={()=>setPoints(p=>p.slice(0,-1))}>Undo point</button><button onClick={()=>setPoints([])}>Clear mask</button><button className="send-button" onClick={save} disabled={saving}>{saving?'Saving…':'Save crop and mask'}</button></div></div></div>
}
...
function Globe({timeMinutes,onCredits}:{timeMinutes:number;onCredits:()=>void}) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const epochRef=useRef(Date.now());epochRef.current=Date.now()-(1440-timeMinutes)*60_000;
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    try { return startGaiaGlobe(canvas,()=>epochRef.current); } catch (error) { console.error('GAIA WebGL failed', error); canvas.classList.add('webgl-failed'); }
  }, []);
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <section className="viewer">
        {view === 'globe' && <Globe timeMinutes={timeMinutes} onCredits={()=>setView('about')}/>} 
