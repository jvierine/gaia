#!/bin/sh
# Add GAIA refit-names support to WISC/AIDA.
#
...
if grep -q applyGaiaProposal "$APP" 2>/dev/null && [ "names" = proposal ]; then
    echo "already applied; nothing to do"; exit 0
fi
if grep -q gaiaStarName "$APP" 2>/dev/null && [ "names" = names ]; then
    echo "already applied; nothing to do"; exit 0
fi
...
if [ "$GOT" != "$EXPECT" ]; then
    ...
    echo "  found    $GOT" >&2
    echo "AIDA has been edited since. Ask Juha to review deploy/aida-names.patch" >&2
