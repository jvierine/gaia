...
\begin{enumerate}
  \item \textbf{The clear-sky reference}, per Sec.~\ref{sec:airmass}. This is
    now the blocking item: the measurements exist, in quantity, and nothing can
    become an optical depth until each star has a clear-sky flux to be compared
    against. The measured spread of $k$ says the per-camera per-night fit is
    the one to build.
