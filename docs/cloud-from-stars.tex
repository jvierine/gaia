...
clear sky near \num{0.20}, a figure the estimator was never told. What still
does not exist is the step from a measurement to a cloud number. No flux has yet
become an optical depth, so no cloud weight reaches the composite and no cloud
map is published. That is now the whole of the remaining work, and none of it is
research: the reference it was waiting for is in place.
\end{abstract}
...
\begin{enumerate}
  \item \textbf{Carry the reference through to a stored optical depth.} This is
    the whole of what is left between the archive and a cloud number. The
    reference exists and is accumulating (Sec.~\ref{sec:reference},
    Sec.~\ref{sec:stall}), and $\tau$ follows from it in one line, but nothing
    yet writes a $\tau$ per measurement. Three things must be decided with it:
    saturated measurements excluded, undetectable ones excluded, and a stated
    fallback for the camera-nights the fit refuses, which are the majority --- no
    weight at all rather than a guessed one.
