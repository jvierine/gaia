...
\date{14 September 2026 \\ \small(third revision; first issued 10 September 2026)}
...
10 September: \num{1.6} million measurements, \num{4761} frames, \num{71}
cameras, every one of them detecting stars. What does not exist is everything
downstream of the measurement. No flux has yet become an optical depth, so no
cloud weight reaches the composite and no cloud map is drawn. The blocking item
is the clear-sky reference, and four days of real series now let us put a number
on why it is hard: regressing flux on air mass gives a median extinction of
\SI{0.38}{mag} per air mass against a clear-sky value near \num{0.20}, because
cloud and air mass are folded into the same fit and cannot be separated by a
whole-series regression.
\end{abstract}
...
\paragraph{What it gives.} Over the archive the guards accept 151 fits and
refuse 745, which is the honest ratio: most camera-nights genuinely do not
constrain extinction. The accepted ones land where they should.
...
\begin{enumerate}
  \item \textbf{Carry the reference through to a stored optical depth.} The
    reference exists (Sec.~\ref{sec:reference}) and $\tau$ follows from it in
    one line, but nothing yet writes a $\tau$ per measurement, and the nights
    the fit refuses need a stated fallback: no weight at all, rather than a
    guessed one.
