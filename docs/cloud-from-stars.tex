...
\date{15 September 2026 \\ \small(fourth revision; first issued 10 September 2026)}
...
faults were found and fixed it is accumulating: 59 fits over 20 cameras at the
time of writing, at \num{0.18} magnitudes per air mass against a literature
clear sky near \num{0.20}, a figure the estimator was never told. Every measurement a fitted
camera-night explains now carries a vertical optical depth, with saturated,
undetectable and unexplained measurements deliberately left null rather than
guessed. What remains is to spend that number: no cloud weight yet reaches the
...
\paragraph{Where no number is claimed.} Three kinds of measurement are left
null rather than given a depth, because in each case a number would be a
fiction. A star looked for and not found has no flux. A star whose peak is
clipped reports an underestimate, and bright aurora makes that common, so its
depth would read as a fading rather than as a full detector. A star
indistinguishable from its own noise is not a measurement, and a fit to noise
returns a small positive flux that a clear-sky reference turns into thin cloud.
...
exceptions are exactly the saturated sample and the one buried in noise.
...
\begin{enumerate}
  \item \textbf{Spend the optical depth.} It is now stored
    (Sec.~\ref{sec:depth}); nothing yet reads it. Two things should:
    \texttt{cloud\_weight} in the composite, and a cell value in the cloud map.
...
\begin{itemize}
  ...
  \item \texttt{backend/main.rs} --- the star endpoints: \texttt{stars},
    \texttt{stars/series}, \texttt{stars/frame} with its camera field outline
