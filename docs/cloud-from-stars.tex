...
offset is dominated by a \emph{rigid shift}, the same in every frame of a
camera, and it differs between cameras.
...
\paragraph{What it actually is.} The offset appeared \emph{after} calibration.
Fitting an affine field to the per-frame offsets and decomposing it separates
the causes. There is a uniform scale term of $5\times10^{-4}$ in both cameras,
about a pixel at the frame edge, which no timing error can produce. There is a
rotation of $10^{-3}$ radians in Bagdarin, which read as a stale timestamp
would mean a lag of 7 to \SI{15}{\second}; Yurga shows none, so a download-time
lag is at most a minor contributor. What is left, and what dominates, is a
constant per-camera pointing shift: the cameras have moved slightly since they
were calibrated. Removing the affine field brings Bagdarin's residual to
\num{0.56} to \SI{0.95}{px}, against the \SI{0.61}{px} scatter of its own
calibration, and Yurga's to \SI{1.3}{px} against its \SI{1.81}{px}. In both
cases the correction recovers the accuracy the calibration itself had.
...
The fix is to fit the offset per frame from the confidently detected stars and
gate on the residual from \emph{that}. A rigid $(\Delta x,\Delta y)$ removes
most of it; the full affine recovers the calibration's own accuracy for four
more parameters on a few tens of stars. Either way it yields a per-frame
pointing check for nothing, and a camera that has drifted far enough to want
recalibrating would announce itself.
