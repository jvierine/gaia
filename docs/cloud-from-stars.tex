...
\date{11 September 2026 \\ \small(revised; first issued 10 September 2026)}
...
and stored per frame. Storage and both diagnostic plots exist. Three things
still do not: nothing yet writes measurements, the cloud weight is not applied
...
\begin{enumerate}
  \item \textbf{A writer.} This is still the blocking item, and everything below
    it is idle until it exists. \texttt{measure\_frame},
    \texttt{record\_frame} and \texttt{record\_sky} are implemented and tested
    but nothing calls them, so both tables are empty and the panel shows no
    data. The pass must decode full-resolution originals, because the
    \SI{256}{px} globe textures destroy stars, so it belongs in a throttled
    background job rather than on a request path.
  ...
  \item \textbf{A server restart.} The star endpoints, and now the camera
    settings changes that carry the quality weight, are live in the code but not
