...
Each camera requests one still every five minutes (fifteen minutes at quota-limited stations), during both day and night. Requests are staggered across each interval, with at most two network image tasks in flight. The separate resumable backfill command uses the same bounded still-image workflow; it is not started automatically by a calibration. Pending tasks are cancelled after 60 seconds. Empty results are reported as unavailable, not stored as images. Failed requests retry on the next scheduled interval, not immediately.
