...
import {ExternalLink,Film,Pause,Play,SlidersHorizontal,X} from 'lucide-react';
...
function MediaDetail({item,eventId,onClose}:{item:EventMedia;eventId:string;onClose:()=>void}){
  ...
  return <aside className="event-detail" aria-label="Selected event image">
    ...
    <div className="event-detail-body">
      ...
      <div className="event-detail-actions"><a className="event-primary" href={aida.href} target="_blank" rel="noreferrer">Calibrate in AIDA <ExternalLink size={14}/></a><a href={item.sourceUrl} target="_blank" rel="noreferrer">Open source posting <ExternalLink size={13}/></a></div>
      <p className="event-rights">{item.license||item.rightsNote||'Copyright remains with the photographer. Research preview only.'}</p>
    </div>
  </aside>;
}

function GlobeMedia({clusters,onChoose}:{clusters:EventCluster[];onChoose:(item:EventMedia)=>void}){
  return <div className="event-globe-media" aria-label="Images in the selected time interval">{clusters.map(cluster=>{
    ...
    return <button key={cluster.items.map(item=>item.id).join('|')} className="event-photo-marker" style={{left:cluster.x,top:cluster.y}} onClick={()=>onChoose(representative)} title={`${cluster.items.length} image${cluster.items.length===1?'':'s'} near ${representative.location}`}>
...
export default function EventStudy(){
  ...
  const [view,setView]=useState<GlobeViewState|null>(null),[selected,setSelected]=useState<EventMedia|null>(null),[source,setSource]=useState('all');
  ...
  useEffect(()=>{if(bins[index])epoch.current=bins[index].at},[bins,index]);
  ...
  return <main className="event-study">
    ...
    <section className="event-stage">
      <GaiaGlobeView className="event-globe" getEpochMillis={()=>epoch.current} mode="base" onViewState={setView}>
        {bin&&<GlobeMedia clusters={clusters} onChoose={setSelected}/>} 
      </GaiaGlobeView>
      <div className="event-status"><span>{bin?`${bin.items.length} observations · ${clusters.length} visible groups`:'Loading event…'}</span><span>Photos are pinned as unprojected previews</span></div>
      {error&&<div className="event-error" role="alert">{error}</div>}
      {selected&&<MediaDetail item={selected} eventId={eventId} onClose={()=>setSelected(null)}/>} 
    </section>
