...
export default function GoogleAccess({children}:{children:React.ReactNode}){
 ...
 return <AccountContext.Provider value={account}>{!session?.email&&<div ref={bar} className="gaia-access" aria-label="Google account"><div ref={button}/>{error&&<span role="alert">{error}</span>}</div>}{ready&&<React.Fragment key={generation}>{children}</React.Fragment>}</AccountContext.Provider>;
}
