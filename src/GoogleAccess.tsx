...
const SignInContext=createContext<React.ReactNode>(null);
export function SignInButton(){return <>{useContext(SignInContext)}</>}
...
export default function GoogleAccess({children}:{children:React.ReactNode}){
 ...
 const signIn=!session?.email?<div className="header-sign-in" aria-label="Google account"><div ref={button}/>{error&&<span role="alert">{error}</span>}</div>:null;
 return <AccountContext.Provider value={account}><SignInContext.Provider value={signIn}>{ready&&<React.Fragment key={generation}>{children}</React.Fragment>}</SignInContext.Provider></AccountContext.Provider>;
}
