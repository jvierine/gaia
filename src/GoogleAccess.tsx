...
export default function GoogleAccess({children}:{children:React.ReactNode}){
 ...
 const apply=(s:Session)=>{setPublicAudience(s.authorized);if(current.current!==s.authorized){current.current=s.authorized;setGeneration(v=>v+1);}setSession(s);setReady(true)};
