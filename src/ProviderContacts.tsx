...
export function ServiceDescription(){return <><p>GAIA — Global Auroral Image Aggregator — compiles low-resolution views from publicly available auroral cameras. Its goal is to provide an overview of auroral activity around the world to support space physics research, with easy access to camera providers and their high-resolution imagery.</p><p><strong>We do not provide high-resolution images or an archival image service.</strong> Playback here is a low-resolution overview, not an archive of original camera observations. Contact the principal investigators (PIs) or operators of the individual cameras for access to high-resolution archival images. Follow the camera links below or click imagery on the globe to visit the originating provider.</p><p><strong>Scientific and commercial use requires permission from the camera operators.</strong> Contact the operators of the individual cameras before using their imagery. Access through GAIA does not grant permission to reuse images; copyright and provider terms remain with the originating producers.</p></>}
export function AuthorAffiliations(){return <><p>Juha Vierinen — Professor of Space Physics, University of Tromsø.</p><p>Björn Gustavsson — Professor of Space Physics, University of Tromsø.</p></>}
const contacts=[
...
export function ProviderContacts(){return <section aria-label="Camera investigators and operators"><h2>Investigators &amp; operators</h2><p>Provider-confirmed roles and data-access links, checked September 2026. A network contact is not necessarily the PI or owner of every camera. Copyright and provider terms remain with the originating producers.</p>{contacts.map(([name,institution,role,url])=><details key={name}><summary>{name}</summary><p><strong>{institution}</strong><br/>{role}<br/><a href={url} target="_blank" rel="noreferrer">Provider information &amp; contact ↗</a></p></details>)}<h3 id="starvisor-camera-credits">STARVISOR camera credits ({starvisorCredits.length} cameras)</h3><p>Installation credits published on the individual station pages. Names, organizations and roles are attributed to STARVISOR; original profile and station links are retained. Scientific and commercial use requires permission from camera operators.</p>{starvisorCredits.map(camera=><article key={camera.source_id}><h4><a href={camera.page_url} target="_blank" rel="noopener noreferrer">{camera.camera} ↗</a></h4><p>{camera.credit||'No camera-specific credit was found on the station page.'}</p>{camera.links.length>0&&<p>{camera.links.map((link,index)=><React.Fragment key={link.url+index}>{index>0?' · ':''}<a href={link.url} target="_blank" rel="noopener noreferrer">{link.name} ↗</a></React.Fragment>)}</p>}</article>)}</section>}
