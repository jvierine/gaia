...
// `cloud` is the per-frame cloud weight field: one small greyscale image in the
// camera's own texture coordinates, sampled alongside the colour and multiplied
// into the weight. It varies across the image, which is why it cannot live in
// `weightScale` with the solar taper and the operator's quality setting.
// PRELIMINARY -- see backend/cloudweight.rs.
export type CameraLayer={geometry:{buffer:WebGLBuffer;count:number};texture:WebGLTexture;cloud?:WebGLTexture;sourceId?:string;weightScale?:number;brightnessGain?:number;order:number;at:string};
export function cameraCompositor(gl:WebGLRenderingContext,forcePortable=false){
  // Camera inputs are at most 256px. Keep vectors/main canvas sharp, but avoid
  // multiple full-retina-sized accumulation buffers for their small textures.
  const rasterSize=(w:number,h:number)=>{const scale=Math.min(1,1024/Math.max(w,h));return [Math.max(1,Math.round(w*scale)),Math.max(1,Math.round(h*scale))]};
...
void main(){vec3 p=rx(-rotation.y)*ry(-rotation.x)*world;w=weight*scale;gl_Position=vec4(p.xy*zoom*min(resolution.x,resolution.y)/resolution,1.-2.*clamp(w,0.000001,0.999999),1.);tc=uv;visible=p.z;}`;
  const mesh=program(vertex,`precision highp float;varying vec2 tc;varying float w;varying float visible;uniform vec2 resolution;uniform sampler2D image;uniform sampler2D previousColor;uniform sampler2D previousWeight;uniform sampler2D cloud;uniform bool hasCloud;uniform float brightnessGain;uniform int mode;uniform vec3 code;
...
void main(){float wc=w*(hasCloud?texture2D(cloud,tc).r:1.);if(visible<0.||wc<=0.)discard;vec2 screen=gl_FragCoord.xy/resolution;vec3 rgb=texture2D(image,tc).rgb*brightnessGain;rgb/=max(1.,max(rgb.r,max(rgb.g,rgb.b)));
if(mode==1){float scale=exp2(texture2D(previousWeight,screen).r*144.-128.);float a=wc/scale;gl_FragColor=vec4(rgb*a,a);}
else if(mode==2){gl_FragColor=vec4(code,1.);}
else if(mode==3){gl_FragColor=vec4(vec3(clamp((log2(wc)+128.)/144.,0.,1.)),1.);}
else if(mode==6){gl_FragColor=vec4(rgb,1.);}
else{float old=decode(texture2D(previousWeight,screen).rgb),total=old+wc;if(mode==4)gl_FragColor=vec4(encode(total),1.);else gl_FragColor=vec4((texture2D(previousColor,screen).rgb*old+rgb*wc)/total,1.);}}`);
  ...
  // White, not the zeroed dummy: a layer with no cloud field must pass through
  // at full weight, and sampling zeros would extinguish it silently.
  const clear=gl.createTexture()!;gl.bindTexture(gl.TEXTURE_2D,clear);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.LINEAR);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,gl.LINEAR);gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,1,1,0,gl.RGBA,gl.UNSIGNED_BYTE,new Uint8Array([255,255,255,255]));
  ...
  let size=[0,0],floating=!forcePortable&&!!(half&&color&&maxBlend),disposed=false,normalizeImages=false,lastKey='';const ids=new WeakMap<object,number>();let nextId=1;const id=(o:object)=>{let v=ids.get(o);if(!v){v=nextId++;ids.set(o,v)}return v};
  const gains=new Map<string,number>();let lastGainTime=performance.now();
  ...
  return {
    ...
    draw(outputW:number,outputH:number,yaw:number,pitch:number,zoom:number,frames:CameraLayer[],textures:Map<string,WebGLTexture>,muted:Set<string>,overlay?:Set<string>|null){
      const now=performance.now(),mix=1-Math.exp(-Math.min(100,now-lastGainTime)/150);lastGainTime=now;
      if(normalizeImages){const visible=new Set<string>();for(const frame of frames){const k=frame.sourceId??String(frame.order),target=frame.brightnessGain??1;visible.add(k);gains.set(k,(gains.get(k)??target)+(target-(gains.get(k)??target))*mix)}for(const k of gains.keys())if(!visible.has(k))gains.delete(k)}
      ...
      const key=[w,h,yaw,pitch,zoom,...frames.filter(f=>!muted.has(f.sourceId||'')).map(f=>`${id(f.geometry.buffer)}:${id(textures.get(f.sourceId??String(f.order))||f.texture)}:${f.weightScale}:${f.cloud?id(f.cloud):0}:${normalizeImages?gains.get(f.sourceId??String(f.order)):1}`)].join(',');
      if(key!==lastKey){
        if(floating){
