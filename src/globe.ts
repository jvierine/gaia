...
export type GaiaGlobeOptions={baseOnly?:boolean;manifestUrl?:string};
export function startGaiaGlobe(canvas: HTMLCanvasElement,getEpochMillis:()=>number,onLoading:(loading:boolean)=>void=()=>{},publicOnly=false,options:GaiaGlobeOptions={}) {
...
void main(){vec2 uv=gl_FragCoord.xy/size;gl_FragColor=mix(texture2D(previous,uv),texture2D(target,uv),amount);}`);
  ...
  void (options.baseOnly?Promise.resolve([]):(publicOnly||options.manifestUrl)?published().then(manifestSources):fetch('/gaia/api/sources',{cache:'no-store',signal:abort.signal}).then(r=>r.json())).then(async (sources:{id:string;name:string;producer:string;calibrated:boolean;enabled:boolean;latitude_deg:number|null;longitude_deg:number|null;quality_exponent?:number}[])=>{
    ...
    addLayer(sites);
    if(!publicOnly&&!options.manifestUrl)cameraSites=sources.filter(s=>s.enabled&&s.latitude_deg!==null&&s.longitude_deg!==null).map(s=>{const lat=s.latitude_deg!*Math.PI/180,lon=s.longitude_deg!*Math.PI/180;return{source_id:s.id,label:`Camera: ${s.name}\n${cameraCredit(s.id,s.producer)}`,world:[Math.cos(lat)*Math.sin(lon),Math.sin(lat),Math.cos(lat)*Math.cos(lon)],lat:s.latitude_deg!,lon:s.longitude_deg!,weight:s.quality_exponent??0}});
    ...
    const catalogue=(id:string)=>{
      let c=catalogues.get(id);if(!c||c.expires<Date.now()){
        if(publicOnly||options.manifestUrl){c={expires:Date.now()+30000,value:published().then(m=>{publicCameras=new Map((m.cameras||[]).filter((c:PublicCamera)=>c.map_index!=null).map((c:PublicCamera)=>[c.map_index!,c]));return browserLayers?m.cameras.find((c:PublicCamera)=>c.source_id===id)?.projection||null:m})};catalogues.set(id,c);return c.value;}
  ...
  }drawLayers(w,h);if(options.baseOnly){const detail={yaw,pitch,zoom,width:canvas.clientWidth,height:canvas.clientHeight},key=[yaw.toFixed(5),pitch.toFixed(5),zoom.toFixed(5),detail.width,detail.height].join(':');if(key!==lastEventView){lastEventView=key;canvas.dispatchEvent(new CustomEvent('gaia-view-state',{detail}))}}measure();animation=requestAnimationFrame(draw)};draw();
