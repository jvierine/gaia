...
void main(){
  ...
  float magneticContours=texture2D(magneticLatitude,mapUv).r*magneticReady;
...
export function startGaiaGlobe(canvas: HTMLCanvasElement,getEpochMillis:()=>number) {
  ...
  void fetch('/gaia/api/igrf-maglat').then(r=>{if(!r.ok)throw new Error(`IGRF grid ${r.status}`);return r.arrayBuffer()}).then(buffer=>{const values=new Uint8Array(buffer);if(values.length!==360*181)throw new Error(`unexpected IGRF grid length ${values.length}`);const contour=new Uint8Array(values.length);for(let i=0;i<values.length;i++){const mag=values[i]/255*180-90;const distance=Math.abs((((mag+15)/30)%1+1)%1-.5)*30;contour[i]=distance<.72?255:0}gl.activeTexture(gl.TEXTURE1);gl.bindTexture(gl.TEXTURE_2D,magneticTexture);gl.pixelStorei(gl.UNPACK_ALIGNMENT,1);gl.texImage2D(gl.TEXTURE_2D,0,gl.LUMINANCE,360,181,0,gl.LUMINANCE,gl.UNSIGNED_BYTE,contour);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.LINEAR);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,gl.LINEAR);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_S,gl.REPEAT);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_T,gl.CLAMP_TO_EDGE);gl.useProgram(program);gl.uniform1f(magneticReady,1)}).catch(console.error);
