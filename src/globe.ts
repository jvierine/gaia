...
precision highp float;
varying float visible;
void main(){if(visible<0.)discard;gl_FragColor=vec4(.97,.48,1.,.90);}
`;
...
function igrfContourVertices(values:Uint8Array){
  const width=360,height=181,levels=[-60,-30,0,30,60],vertices:number[]=[];
  const mag=(x:number,y:number)=>values[y*width+(x+width)%width]/255*180-90;
  const crossing=(x1:number,y1:number,v1:number,x2:number,y2:number,v2:number,level:number)=>{const d=v2-v1,t=Math.abs(d)<1e-9?.5:(level-v1)/d;return[(x1+(x2-x1)*t-180)*Math.PI/180,(90-(y1+(y2-y1)*t))*Math.PI/180]};
...
precision highp float;varying vec3 color;varying float visible;void main(){if(visible<0.)discard;gl_FragColor=vec4(color,1.);}`);
  ...
  const credits=document.createElement('div');credits.style.cssText='position:absolute;left:18px;top:18px;max-width:70%;color:#d6eee8;background:#031018cc;padding:8px 12px;font:11px monospace;pointer-events:none';credits.textContent='Camera points: green calibrated · red uncalibrated · gray disabled';canvas.parentElement?.appendChild(credits);
