...
await Promise.all(['thumbnails','previews'].map(name=>mkdir(path.join(output,name),{recursive:true})));
...
async function worker(){while(cursor<candidates.length){const index=cursor++,{row,capturedAt,capturedUntil}=candidates[index],name=key(row.record_id),preview=path.join(output,'previews',`${name}.jpg`),thumbnail=path.join(output,'thumbnails',`${name}.jpg`),video=row.media_type==='timelapse video';
    try{
      ...
      items[index]={id:row.record_id,source:row.source,kind:video?'timelapse':'image',title:row.title||null,creator:row.creator||null,location:row.location_text||row.geocode_display_name||'Location unavailable',latitude:row.latitude,longitude:row.longitude,capturedAt,capturedUntil,timePrecision:row.time_precision||'unknown',timeSource:row.time_source||'unknown',coordinateSource:row.coordinate_source||'unknown',coordinatePrecision:row.coordinate_precision||'unknown',grade:row.spatiotemporal_grade||'ungraded',thumbnailUrl:`${baseUrl}/thumbnails/${name}.jpg`,previewUrl:`${baseUrl}/previews/${name}.jpg`,mediaUrl:null,sourceUrl:row.source_url||row.original_url,originalUrl:row.original_url||null,license:row.license||null,rightsNote:row.rights_note||null};
